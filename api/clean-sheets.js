...
export default async function handler(req,res){
  ...
  try{
    ...
    const players=list.map((item,index)=>({
      ...
      name:val(item,'ParticipantName','participantName','playerName','name')||'Goalkeeper',
      team:val(item,'TeamName','teamName','team')||'',
      photo:'',
      teamLogo:'',
      cleanSheets:Number(val(item,'StatValue','statValue','value'))||0,
      appearances:Number(val(item,'MatchesPlayed','matchesPlayed','played'))||0
    })).filter(p=>p.name).sort((a,b)=>b.cleanSheets-a.cleanSheets||a.rank-b.rank).slice(0,20).map((p,i)=>({...p,rank:i+1}));
    ...
    return res.status(200).json({season:'2026/27',updatedAt:new Date().toISOString(),source:'FotMob',players,note:'Goalkeeper clean sheets for the current Premier League season.'});
  }catch(error){
