...
async function loadStat(stat){
  ...
  return rows.map((p,i)=>({rank:Number(p.rank)||i+1,id:p.id,name:p.name||'Unknown player',teamId:p.teamId,value:Number(p.statValue?.value??0)})).sort((a,b)=>b.value-a.value||a.rank-b.rank);
}
