...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const image=`${SITE}api/instagram-story-image?src=${encodeURIComponent(SRC)}&title=${encodeURIComponent(TITLE)}&v=2`;
    const text=`⚽ FOOTBALL TALK\n\n${TITLE}\n\nA second controlled image-quality test using the upgraded Football Talk Instagram renderer.\n\n💬 What’s your verdict? Have your say 👇\n\nfootballtalk.uk\n\n#FootballTalk #WhereFansHaveTheirSay`;
  ...
  }catch(e){console.error('Instagram test push 2 failed',e);return res.status(502).json({ok:false,error:String(e.message||e)});}
};
