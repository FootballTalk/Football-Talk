...
async function pendingState(cfg,items){const state=[];for(const item of items){if(!item?.id||item.enabled===false)continue;const done={};for(const service of['facebook','instagram'])done[service]=await alreadyRecorded(cfg,item.id,service);state.push({id:item.id,title:item.title||'',done});}return state;}
async function firstPending(cfg,items){for(const item of items){if(!item?.id||item.enabled===false)continue;const services=[];for(const service of['facebook','instagram'])if(!(await alreadyRecorded(cfg,item.id,service)))services.push(service);if(services.length)return{item,services};}return null;}
...
async function publishPending(){
  ...
  const targets={facebook:channelFor(info,'facebook'),instagram:channelFor(info,'instagram')};
