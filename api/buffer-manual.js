...
function imageFor(item,service){if(service==='instagram')return INSTAGRAM_SOCIAL_IMAGE;if(service==='facebook'){const src=item.image||DEFAULT_SOCIAL_IMAGE;return`${SITE_URL}api/facebook-story-image?src=${encodeURIComponent(src)}&story=${encodeURIComponent(item.id||'manual')}`;}return item.image||DEFAULT_SOCIAL_IMAGE;}
