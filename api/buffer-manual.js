...
function channelFor(info,service){return(info.channels||[]).find(c=>{const actual=String(c.service||'').toLowerCase(),serviceMatch=service==='twitter'?(actual==='twitter'||actual==='x'):actual===service;if(!serviceMatch)return false;const label=`${c.name||''} ${c.displayName||''}`;return service==='facebook'?/football\s*talk/i.test(label):true;});}
...
async function pendingState(cfg,items){const state=[];for(const item of items){if(!item?.id||item.enabled===false)continue;const done={};for(const service of['facebook'])done[service]=await alreadyRecorded(cfg,item.id,service);state.push({id:item.id,title:item.title||'',done});}return state;}
async function firstPending(cfg,items){for(const item of items){if(!item?.id||item.enabled===false)continue;const services=[];for(const service of['facebook'])if(!(await alreadyRecorded(cfg,item.id,service)))services.push(service);if(services.length)return{item,services};}return null;}
...
async function publishPending(){
  ...
  const targets={facebook:channelFor(info,'facebook')};
