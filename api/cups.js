...
function applyOfficialFixtureOverrides(fixture){if(Number(fixture.leagueId)===48&&fixture.home==='Liverpool'&&fixture.away==='Tottenham Hotspur'&&fixture.status==='NS'){return{...fixture,date:'2026-09-15T19:00:00Z',timestamp:1789498800,officialOverride:true};}return fixture;}
function mapFixture(item){return{id:item.fixture?.id,date:item.fixture?.date,timestamp:item.fixture?.timestamp,status:item.fixture?.status?.short||'NS',elapsed:item.fixture?.status?.elapsed??null,round:item.league?.round||'',home:item.teams?.home?.name||'',away:item.teams?.away?.name||'',homeLogo:item.teams?.home?.logo||'',awayLogo:item.teams?.away?.logo||'',homeGoals:item.goals?.home??null,awayGoals:item.goals?.away??null,leagueId:Number(item.league?.id)||null};}
...
async function fetchCup(cup,apiKey,season,from,to){const url=new URL(`${API_BASE}/fixtures`);url.searchParams.set('league',String(cup.id));url.searchParams.set('season',String(season));url.searchParams.set('from',from);url.searchParams.set('to',to);url.searchParams.set('timezone','Europe/London');const rows=await readApi(url,apiKey,cup.name);return{id:cup.id,name:cup.name,fixtures:rows.map(mapFixture).map(applyOfficialFixtureOverrides).filter(f=>f.date&&f.home&&f.away).sort((a,b)=>(a.timestamp||0)-(b.timestamp||0))};}
async function fetchLive(apiKey){const url=new URL(`${API_BASE}/fixtures`);url.searchParams.set('live','all');url.searchParams.set('timezone','Europe/London');const rows=await readApi(url,apiKey,'Live cup scores');return CUPS.map(cup=>({...cup,fixtures:rows.filter(item=>Number(item.league?.id)===cup.id).map(mapFixture).map(applyOfficialFixtureOverrides)}));}

async function fotmobFallback(res,liveOnly,reason=''){
  const now=new Date();
  const fromMs=now.getTime()-PAST_DAYS*24*60*60*1000;
  const toMs=now.getTime()+FUTURE_DAYS*24*60*60*1000;
  const fetched=await Promise.all(FOTMOB_CUPS.map(async cup=>({cup,fixtures:await getFotmobLeagueMatches(cup)})));
  const cups=fetched.map(({cup,fixtures})=>({id:cup.siteId,name:cup.name,fixtures:(liveOnly?fixtures.filter(f=>LIVE.has(f.status)):withinRange(fixtures,fromMs,toMs)).map(applyOfficialFixtureOverrides).sort((a,b)=>(a.timestamp||0)-(b.timestamp||0))}));
...
export default async function handler(req,res){
  ...
  const key=(process.env.API_FOOTBALL_KEY||'').trim();const now=new Date();const fromDate=new Date(now.getTime()-PAST_DAYS*24*60*60*1000);const toDate=new Date(now.getTime()+FUTURE_DAYS*24*60*60*1000);const from=londonDateString(fromDate);const to=londonDateString(toDate);const season=seasonFor(now);const liveOnly=String(req.query?.live||'')==='1';
