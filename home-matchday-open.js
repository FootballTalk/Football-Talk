(function setupAlwaysOpenHomeMatchday(){
  ...
  const approvedNames=new Map([[39,/^premier league$/i],[40,/^(efl )?championship$/i],[41,/^(efl )?league one$/i],[42,/^(efl )?league two$/i],[45,/^fa cup$/i],[48,/^(carabao cup|efl cup)$/i],[2,/^(uefa )?champions league$/i],[3,/^(uefa )?europa league$/i],[848,/^(uefa )?conference league$/i],[61,/^ligue 1$/i],[71,/^serie a$/i],[87,/^laliga$/i],[78,/^bundesliga$/i],[55,/^liga portugal$/i],[135,/^serie a$/i]]);
  let loading=false;
  ...
  function ensureShell(){
    ...
    section.innerHTML=`<div class="ft-ln-shell"><div class="ft-ln-head"><div><div id="ft-ln-kicker" class="ft-ln-kicker">FT LIVE</div><h2 id="ft-ln-title">MATCHDAY LIVE</h2><p id="ft-ln-sub" class="ft-ln-sub">Today’s football stays open here all day — scheduled kick-offs, live scores and full-time results.</p></div><a class="ft-ln-link" href="match-centre.html">FULL MATCH CENTRE →</a></div><div id="ft-ln-list" class="ft-ln-list"><div class="ft-ln-empty">Loading today’s matches…</div></div><div class="ft-ln-footer"><span id="ft-ln-state">Scores refresh automatically every 30 seconds.</span><span id="ft-ln-updated"></span></div></div>`;
    return section;
  }

  function addStyles(){if(document.getElementById('ft-home-open-styles'))return;const styles=document.createElement('style');styles.id='ft-home-open-styles';styles.textContent=`#ft-live-now{max-width:1180px;margin:18px auto 8px;padding:0 20px}.ft-ln-shell{background:#0b0b0e;color:#fff;border-radius:18px;overflow:hidden;border-top:6px solid #f7c600;box-shadow:0 14px 34px rgba(0,0,0,.18)}.ft-ln-head{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:18px 20px;border-bottom:1px solid #29292f}.ft-ln-kicker{font-size:12px;font-weight:1000;letter-spacing:.12em;color:#f7c600}.ft-ln-head h2{margin:2px 0 4px;font-size:clamp(25px,4vw,38px)}.ft-ln-sub{margin:0;color:#b8b8c2;font-size:13px}.ft-ln-link{background:#f7c600;color:#090909!important;text-decoration:none;font-weight:1000;padding:11px 14px;border-radius:9px;white-space:nowrap}.ft-ln-list{padding:14px 16px 16px}.ft-ln-league{margin-bottom:16px}.ft-ln-league-title{color:#f7c600;font-size:12px;font-weight:1000;letter-spacing:.06em;margin:0 0 7px}.ft-ln-game{display:grid;grid-template-columns:minmax(0,1fr) 92px minmax(0,1fr);align-items:center;gap:9px;background:#fff;color:#111;padding:11px 12px;margin-bottom:8px;border-radius:10px}.ft-ln-team{font-weight:900}.ft-ln-home{text-align:right}.ft-ln-centre{text-align:center}.ft-ln-score{display:block;font-family:'Archivo Black',sans-serif;background:#111;color:#fff;padding:7px 5px;border-radius:7px;font-size:16px}.ft-ln-status{display:block;font-size:10px;font-weight:1000;margin-top:4px;color:#777}.ft-ln-status.live{color:#c80000}.ft-ln-empty{background:#fff;color:#555;padding:18px;border-radius:10px;text-align:center}.ft-ln-footer{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:0 16px 16px;color:#aaa;font-size:11px}.ft-ln-live-dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:#d90000;margin-right:6px;box-shadow:0 0 0 4px rgba(217,0,0,.15)}@media(max-width:650px){#ft-live-now{padding:0 14px}.ft-ln-head{align-items:flex-start;flex-direction:column}.ft-ln-link{width:100%;text-align:center}.ft-ln-game{grid-template-columns:minmax(0,1fr) 68px minmax(0,1fr);padding:10px 7px}.ft-ln-team{font-size:12px}.ft-ln-footer{align-items:flex-start;flex-direction:column}}`;document.head.appendChild(styles)}
  ...
  function render(leagues){
    ...
    if(liveCount){kicker.innerHTML='<span class="ft-ln-live-dot"></span>FT LIVE';title.textContent='MATCHDAY LIVE · LIVE NOW';sub.textContent='Today’s fixtures remain open on the homepage while live scores and match status update automatically.';state.textContent=`${liveCount} match${liveCount===1?'':'es'} live now · refreshing every 30 seconds.`}
    else{kicker.textContent='FT LIVE';title.textContent='MATCHDAY LIVE';sub.textContent='Today’s football stays open here all day — scheduled kick-offs, live scores and full-time results.';state.textContent='Scores refresh automatically every 30 seconds.'}
    list.replaceChildren();
    if(!wanted.length){const e=document.createElement('div');e.className='ft-ln-empty';e.textContent='No featured fixtures are scheduled today. The live board stays here ready for the next matchday.';list.appendChild(e)}
