(function setupAlwaysOpenHomeMatchday(){
  ...
  function setExpanded(section,expanded){
    const body=section.querySelector('#ft-ln-body');
    const button=section.querySelector('#ft-ln-toggle');
    if(!body||!button)return;
    body.hidden=!expanded;
    body.style.display=expanded?'block':'none';
    button.setAttribute('aria-expanded',String(expanded));
    button.textContent=expanded?'HIDE LIVE SCORES ▲':'SHOW LIVE SCORES ▼';
  ...
  function render(leagues){
    ...
    if(!wanted.length){const e=document.createElement('div');e.className='ft-ln-empty';e.textContent='No featured fixtures are scheduled today. The live board is ready for the next matchday.';list.appendChild(e)}
