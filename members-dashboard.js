(() => {
  ...
  function updateNotificationUI(){
    const btn=document.getElementById('enable-notify');
    const note=document.getElementById('notify-note');
    if(!btn)return;
    if(!notificationsSupported()){
      btn.textContent='NOT AVAILABLE';btn.disabled=true;
      if(note)note.textContent='This browser does not support website notifications. You can still use the Members Area normally.';
      return;
    }
    if(Notification.permission==='granted'){
      btn.textContent='ON';btn.disabled=true;
      if(note)note.textContent='Browser notifications are enabled on this device.';
    }else if(Notification.permission==='denied'){
      btn.textContent='BLOCKED';btn.disabled=true;
      if(note)note.textContent='Notifications are blocked in this browser’s site settings.';
    }else{
      btn.textContent='ENABLE';btn.disabled=false;
      if(note)note.textContent='Optional — your browser will ask for permission.';
    }
  }
  function notifyTest(){
    if(!notificationsSupported()){updateNotificationUI();return}
    Notification.requestPermission().then(p=>{
      updateNotificationUI();
      if(p==='granted')new Notification('Football Talk',{body:'Member notifications are switched on. ⚽'});
  ...
  function wireTransferAlerts(){
    ...
    let known='';const observer=new MutationObserver(()=>{const p=prefs();if(!p.notifyTransfers||Notification.permission!=='granted')return;const item=document.querySelector('#members-transfer-feed .itsago-live-item');if(!item)return;const text=item.textContent.trim();if(known&&text!==known)new Notification('🚨 IT’S A GO!',{body:text.slice(0,180)});known=text;});
