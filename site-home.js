document.addEventListener('DOMContentLoaded',async()=>{
  ...
  const teamLine=(name,logo)=>`<div class="ft-match-team">${logo?`<img class="ft-match-badge" src="${esc(logo)}" alt="${esc(name)} badge" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()">`:''}<span>${esc(name)}</span></div>`;
  try{
    ...
    box.innerHTML=games.slice(0,8).map(g=>{
      ...
      return `<div class="ft-match"><div class="ft-match-time ${live?'live':''}">${time}</div><div><div class="ft-match-teams">${teamLine(g.home,g.homeLogo)}${teamLine(g.away,g.awayLogo)}</div><div class="ft-match-league">${esc(g.leagueName||'')}</div></div><div class="ft-match-score">${score}</div></div>`;
    }).join('');
