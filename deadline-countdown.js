(()=>{
  ...
  const split=ms=>{const safe=Math.max(0,ms);const t=Math.floor(safe/1000);return{days:Math.floor(t/86400),hours:Math.floor((t%86400)/3600),mins:Math.floor((t%3600)/60),secs:t%60,millis:Math.floor(safe%1000)}};
    ...
    const tick=()=>{
      ...
      if(diff<=0){
        if(!closed){
          ...
          section.querySelector('[data-tdc="millis"]').textContent='000';
        }
        return;
      }
      const t=split(diff);
      ...
      section.querySelector('[data-tdc="millis"]').textContent=padMs(t.millis);
